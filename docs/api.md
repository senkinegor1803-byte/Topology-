...
В `DEFAULT_PIPELINE` четыре шага — все реальные, переиспользуют уже
реализованные Шаги 1.1/1.2/1.4-1.8, а не заглушки:
...
Веб-конвертация (IFC → GLB, Шаг 1.9) добавится в `DEFAULT_PIPELINE` по мере
реализации, не меняя движок оркестрации (`jobs/pipeline.py`).
...
- **Реальные шаги** (`jobs/steps.py`) — `select_osm`/`prepare_relief`/
  `select_and_normalize`/`assemble_ifc` против настоящего `osm2pgsql`-импорта
  и настоящего слияния рельефа; точные сообщения об ошибках подтверждены.
...
Итого по Шагу 1.3 (с учётом `assemble_ifc`, Шаг 1.8) — все тесты пакета
(247/247 на момент Шага 1.8) в общем прогоне; `test_storage`, `test_jobs_*`,
`test_tasks_celery`, `test_api`, `test_ifc_assemble`, `test_ifc_registry*`.

## Чего не хватает (вне возможностей AI-сессии)

- Настоящего MinIO (нет демона Docker) — интерфейс готов, подключение только
  конфигурационное.
- Веб-конвертации (IFC → GLB, Шаг 1.9) в пайплайне.
- Нагрузочной проверки на реальных объёмах (существенно больше 10 задач,
  реальные размеры IFC/растров) — имеет смысл после Шага 1.9, когда пайплайн
  станет полным, и после выбора пилота (Шаг 0.1).

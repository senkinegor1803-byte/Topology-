...
Сейчас в `DEFAULT_PIPELINE` два шага — оба реальные, переиспользуют уже
реализованные Шаги 1.1/1.2, а не заглушки:

1. **`select_osm`** — `topology_geo.osm.queries.count_within_radius` (Шаг 1.1)
   по центру/радиусу задачи; результат — JSON со сводкой по слоям, сохранён в
   Storage. Если таблиц OSM нет (импорт не запускался) — ошибка «нет данных
   OSM для этой области».
...
Дальнейшие шаги конвейера (выборка/TIN участка, здания, дороги, сборка IFC,
веб-конвертация — Шаги 1.4–1.9) добавятся в `DEFAULT_PIPELINE` по мере
...
- **Реальные шаги** (`jobs/steps.py`) — `select_osm`/`prepare_relief` против
  настоящего `osm2pgsql`-импорта и настоящего слияния рельефа; точные
  сообщения об ошибках подтверждены.
...
Итого по Шагу 1.3 — 34 новых теста (`test_storage`, `test_jobs_*`,
`test_tasks_celery`, `test_api`), все в общем прогоне пакета (114/114).
...
- Нагрузочной проверки на реальных объёмах (существенно больше 10 задач,
  реальные размеры IFC/растров) — имеет смысл после Шагов 1.4-1.9, когда

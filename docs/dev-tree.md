...
| Шаг | Статус | Где | Примечание |
| --- | --- | --- | --- |
| 1.1 Импорт OSM в PostGIS | ⬜ | — | Требует поднятый PostGIS (0.7) и реальную выгрузку OSM. |
| 1.2 Подготовка рельефа | ⬜ | — | Формулы сшивки — `docs/math-model.md` §2.4. |
...
| --- | --- | --- |
| CI (lint + тесты) | ✅ | `.github/workflows/ci.yml` | ruff + pytest (40 тестов) + `docker compose config` на каждый push/PR. |

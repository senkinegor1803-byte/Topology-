...
flowchart TD
    ...
    subgraph E1["Этап 1 — MVP 500 м"]
        ...
        S14["1.4 Выборка участка"]:::notstarted
...
| 1.3 API и очередь задач | ✅(частично) | `geo/src/topology_geo/api/*.py`, `geo/src/topology_geo/jobs/*.py`, `geo/src/topology_geo/tasks/*.py`, `docs/api.md` | FastAPI (`POST /jobs`, `GET /jobs/{id}`, `GET /models/{id}/files`), модель задач в PostGIS, реальная очередь Celery+Redis (проверена настоящим воркером в отдельном процессе), пайплайн из двух реальных шагов (select_osm/prepare_relief, поверх Шагов 1.1/1.2). Оба критерия приёмки шага подтверждены тестами: 10 параллельных задач не мешают друг другу, упавший шаг перезапускается без пересчёта предыдущих. 34 теста, все зелёные. Не хватает реального MinIO (нет демона Docker — интерфейс подключаемый) и шагов 1.4-1.9 в самом пайплайне (появятся по мере реализации). |
| 1.4 Выборка и нормализация данных | ⬜ | — | Формула буфера — `docs/math-model.md` §2.1 (bbox-часть уже переиспользована в 1.3). |
...
| --- | --- | --- |
| CI (lint + тесты) | ✅ | `.github/workflows/ci.yml` — ruff + pytest (114 тестов, включая интеграционные с сервисами PostGIS + Redis, osm2pgsql, реальный Celery-воркер) + `docker compose config` на каждый push/PR. |

...
| --- | --- | --- |
| CI (lint + тесты) | ✅ | `.github/workflows/ci.yml` — ruff + pytest (258 тестов, включая интеграционные с сервисами PostGIS + Redis, osm2pgsql, реальный Celery-воркер, реальный headless Chromium для вьюера) + `docker compose config` на каждый push/PR. CI-раннер должен иметь Chromium (`playwright install chromium` или системный) — иначе браузерные тесты вьюера пропускаются. |

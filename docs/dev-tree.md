...
flowchart TD
    ...
    subgraph E2["Этап 2 — 3 км + карта города"]
        ...
        S22["2.2 Здания LOD2"]:::notstarted
...
| --- | --- | --- |
| CI (lint + тесты) | ✅ | `.github/workflows/ci.yml` — ruff + pytest (292 теста, включая интеграционные с сервисами PostGIS + Redis, osm2pgsql, реальный Celery-воркер (включая параллельную генерацию тайлов, `--concurrency=4`), реальный headless Chromium для вьюера, сквозной прогон на 3 профилях участка) + `docker compose config` на каждый push/PR. CI-раннер должен иметь Chromium (`playwright install chromium` или системный) — иначе браузерные тесты вьюера пропускаются. |
